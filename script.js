...
let playerName = "";
let currentQuestions = []; // Ini akan berisi 15 soal yang SUDAH DIURUTKAN
let customGameQuestions = []; // Ini juga akan diurutkan
...
// --- Elemen DOM ---
// (Tidak ada perubahan di sini, semua elemen DOM sama)
    ...
    if (hash) {
        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                // Urutkan soal kustom berdasarkan kesulitan
                const sortedQuestions = sortQuestionsByDifficulty(allQuestions);
                
                if (sortedQuestions.length < 15) {
                    console.warn(`Kuis kustom hanya memiliki ${sortedQuestions.length} soal. Butuh 5 per level.`);
                    customGameStatus.textContent = `Error: Kuis kustom tidak lengkap. Butuh min. 5 soal per level.`;
                    startBtn.disabled = true;
                    return;
                }

                customGameQuestions = sortedQuestions; // Simpan soal yang sudah diurutkan
...
async function startGame() {
    ...
    try {
        ...
        if (customGameId && customGameQuestions.length > 0) {
            // --- ALUR 1: KUIS KUSTOM (dari Firebase) ---
            // Soal sudah diambil dan diurutkan oleh checkURLHash()
            console.log("Memuat kuis kustom...");
            currentQuestions = customGameQuestions; // Langsung gunakan soal yang sudah diurutkan
        
        } else if (!customGameId) {
            // --- ALUR 2: KUIS DEFAULT (dari file CSV lokal) ---
            ...
            allQuestions = parseCSV(csvData); // Ambil semua soal
            
            // Urutkan soal default berdasarkan kesulitan
            const sortedQuestions = sortQuestionsByDifficulty(allQuestions);
            ...
            currentQuestions = sortedQuestions; // Simpan 15 soal yang sudah diurutkan
        } else {
        ...
        buildPrizeLadder();
        showQuestion(); // Tampilkan soal pertama (soal mudah)
...
// --- FUNGSI BARU UNTUK MENGURUTKAN SOAL ---
function sortQuestionsByDifficulty(allQuestions) {
    // 1. Pisahkan soal ke 3 kolam
    const easyPool = [];
    const mediumPool = [];
    const hardPool = [];
    ...
    if (easyPool.length < 5 || mediumPool.length < 5 || hardPool.length < 5) {
        ...
        return []; // Kembalikan array kosong untuk memicu error di startGame
    }

    // 4. Buat daftar 15 soal yang sudah diurutkan
    const finalQuestionList = [
        ...easyPool.slice(0, 5),   // 5 soal mudah pertama
        ...mediumPool.slice(0, 5), // 5 soal sedang
        ...hardPool.slice(0, 5)    // 5 soal sulit
    ];
...
function generatePhoneAnswer(contactType) {
    ...
    optionButtons.forEach(btn => {
        if (!btn.classList.contains('disabled')) {
            visibleOptions.push(btn.dataset.answer);
        }
    });
    const wrongOptions = visibleOptions.filter(opt => opt !== correctAnswer);
    let finalAnswer;
    let reliability = (contactType === 'expert') ? 0.9 : 0.6;
    let name = (contactType === 'expert') ? "Ahli" : "Teman";
...
// --- Fungsi Host (Kustom) ---
// (Tidak ada perubahan di sini, parseCSV sudah diupdate)
async function generateCustomGame() {
    ...
    try {
        ...
        const csvData = await response.text();
        const questions = parseCSV(csvData); // Otomatis baca 4 kolom
        if (questions.length === 0) {
            throw new Error("Tidak ada soal yang ditemukan di CSV. Pastikan formatnya benar (q,o,a,difficulty).");
        }
        // Validasi cepat
        ...
        // Simpan ke Firebase (soal belum diurutkan, akan diurutkan saat player join)
...
// ***** PERUBAHAN DI SINI *****
// Parser CSV (digunakan oleh Host dan Kuis Default)
function parseCSV(csvData) {
    const questions = [];
    const lines = csvData.split(/\r?\n/); 
    
    // Mulai dari 1 untuk melewati header (q,o,a,difficulty)
    for (let i = 1; i < lines.length; i++) {
        ...
        if (parts.length >= 4) { // HARUS PUNYA 4 KOLOM
            const q = parts[0].trim().replace(/"/g, ''); 
            const o = parts[1].trim().replace(/"/g, ''); 
            const a = parts[2].trim().replace(/"/g, ''); 
            const difficulty = parseInt(parts[3].trim().replace(/"/g, ''), 10); // Kolom ke-4
...
function showQuestion() {
    resetState();
    // Kita tidak perlu cek '>= currentQuestions.length' di sini
    // karena 'processAnswer' akan menangkap 'gameWin'
    
    const questionData = currentQuestions[currentQuestionIndex];
    questionText.textContent = questionData.q;
    let options = parseOptions(questionData.o);
    options = shuffleArray(options); // Opsi jawaban tetap diacak
    optionButtons.forEach((btn, index) => {
        btn.querySelector('p').textContent = options[index];
        btn.dataset.answer = options[index]; 
    });
    updatePrizeLadder(); // Update tangga hadiah ke level saat ini
    startTimer(60);
}
...
function processAnswer() {
    ...
    if (selectedAnswer === correctAnswer) {
        ...
        // Cek apakah sudah menang (menyelesaikan soal ke-15)
        if (currentQuestionIndex === prizeLadderDisplay.length) { // index 14 (soal ke-15)
            ...
            currentScore = prizeLadderDisplay[currentQuestionIndex - 1]; // Update skor sementara
            currentScoreValue = prizeLadderValues[currentQuestionIndex - 1]; 
        ...
        // Tentukan skor aman
        currentScoreValue = 0; // Default
        if (currentQuestionIndex > 9) currentScoreValue = prizeLadderValues[9]; // Skor aman 32jt (index 9)
        else if (currentQuestionIndex > 4) currentScoreValue = prizeLadderValues[4]; // Skor aman 1jt (index 4)
        
        // Sesuaikan skor tampilan
...
function gameWin() {
    bgAudio.pause();
    // Skor sudah diatur di 'processAnswer'
...
function parseOptions(optionsString) {
    return optionsString.split(',').map(opt => opt.replace('.', ''));
}
...
function buildPrizeLadder() {
    ...
    const safePoints = [4, 9, 14]; // Index 4 (1jt), 9 (32jt), 14 (1M)
